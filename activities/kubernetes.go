...
func (k *KubernetesActivities) RollbackDeployment(ctx context.Context, req shared.RollbackDeploymentRequest) (*shared.RollbackDeploymentResponse, error) {
	...
	checkCmd := exec.Command("kubectl", "get", "deployment", deploymentName, "-n", namespace)
	if err := checkCmd.Run(); err != nil {
		logger.Warn("Deployment not found, nothing to rollback")
		return &shared.RollbackDeploymentResponse{
			Success: true,
			Message: fmt.Sprintf("No deployment found in %s environment to rollback", req.Environment),
		}, nil
	}
	activity.RecordHeartbeat(ctx, "Deployment found")

	// Step 2: Perform kubectl rollout undo
	logger.Info("[Rollback 2/4] Performing rollback to previous revision")
	rollbackCmd := exec.Command("kubectl", "rollout", "undo", 
		fmt.Sprintf("deployment/%s", deploymentName), "-n", namespace)
	
	var rollbackOut bytes.Buffer
	var rollbackErr bytes.Buffer
	rollbackCmd.Stdout = &rollbackOut
	rollbackCmd.Stderr = &rollbackErr
	
	if err := rollbackCmd.Run(); err != nil {
		// If rollback fails, delete the deployment instead
		logger.Warn("Rollback failed, deleting deployment instead", "error", err, "stderr", rollbackErr.String())
		return k.deleteDeployment(ctx, deploymentName, namespace, req.Environment)
	}
	
	logger.Info("Rollback initiated", "output", rollbackOut.String())
	activity.RecordHeartbeat(ctx, "Rollback initiated")
	...
	statusCmd := exec.Command("kubectl", "rollout", "status", 
		fmt.Sprintf("deployment/%s", deploymentName), "-n", namespace, "--timeout=60s")
	...
	if err := statusCmd.Run(); err != nil {
		logger.Warn("Rollback status check failed, proceeding anyway", "error", err, "stderr", statusErr.String())
	} else {
		logger.Info("Rollback status", "output", statusOut.String())
	}
	activity.RecordHeartbeat(ctx, "Rollback status checked")

	// Step 4: Verify rollback success
	logger.Info("[Rollback 4/4] Verifying rollback success")
	verifyCmd := exec.Command("kubectl", "get", "deployment", deploymentName, "-n", namespace, "-o", "wide")
	...
	if err := verifyCmd.Run(); err != nil {
		logger.Error("Failed to verify rollback", "error", err, "stderr", verifyErr.String())
		return &shared.RollbackDeploymentResponse{
			Success: false,
			Message: fmt.Sprintf("Rollback verification failed for %s deployment: %s", req.Environment, verifyErr.String()),
		}, nil
	}
	
	logger.Info("Rollback verification", "output", verifyOut.String())
	logger.Info("Deployment rollback completed successfully")

	return &shared.RollbackDeploymentResponse{
		Success: true,
		Message: fmt.Sprintf("Successfully rolled back %s deployment to previous revision", req.Environment),
	}, nil
}

// deleteDeployment deletes a deployment when rollback is not possible
func (k *KubernetesActivities) deleteDeployment(ctx context.Context, deploymentName, namespace, environment string) (*shared.RollbackDeploymentResponse, error) {
	logger := activity.GetLogger(ctx)
	
	logger.Info("Deleting deployment as fallback rollback method")
	
	// Delete deployment
	deleteCmd := exec.Command("kubectl", "delete", "deployment", deploymentName, "-n", namespace)
	...
	if err := deleteCmd.Run(); err != nil {
		logger.Error("Failed to delete deployment", "error", err, "stderr", deleteErr.String())
		return &shared.RollbackDeploymentResponse{
			Success: false,
			Message: fmt.Sprintf("Failed to delete %s deployment: %s", environment, deleteErr.String()),
		}, nil
	}
	
	logger.Info("Deployment deleted", "output", deleteOut.String())
	
	return &shared.RollbackDeploymentResponse{
		Success: true,
		Message: fmt.Sprintf("Successfully deleted %s deployment (rollback via deletion)", environment),
	}, nil
}
